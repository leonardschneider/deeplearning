...
module type network = {
  ...
  val train [K] 'w 'g 'o 'i 'state [s] [ps] [ts]:
            NN i w o g t [s] [ps] [ts] ->
            w ->
            Opt [ts] t state w ->
            state ->
            (lr: f32) ->
  ...
  val connect_layers 'w1 'w2 'i1 'o1 'o2 'c1 'c2 [s1] [s2] [p1] [p2] [ts1] [ts2]:
                      NN i1 w1 o1 c1 t [s1] [p1] [ts1] ->
                      NN o1 w2 o2 c2 t [s2] [p2] [ts2] ->
                      NN i1 (w1, w2) (o2) (c1, c2) t [s1+s2] [p1+p2] [ts1+ts2]

  val size 'w 'g 'i 'e1 'e2 '^u 'o [s] [p] [ts]: NN i w o g t [s] [p] [ts] -> i64

  --- Initializes the weights of a network
  val init_weights 'w 'g 'i 'o [s] [p] [ts]:
    NN i w o g t [s] [p] [ts] -> weights (*w)

  --- Performs predictions on data set given a network,
  --- input data and activation func
  val predict [K] 'w 'g 'i 'o [s] [p] [ts]: NN i w o g t [s] [p] [ts] ->
                                              weights w ->
                                             [K]i ->
                                             activation_func o ->
                                             [K]o
...
module neural_network (R:real): network with t = R.t = {
  ...
  let connect_layers 'w1 'w2 'i1 'o1 'o2 'c1 'c2 [s1] [s2] [ps1] [ps2] [ts1] [ts2]
                     ({forward=f1, backward=b1,
                        pickle=p1, specs=sp1,
                        functor=fun1, w_init=wi1,
                        update_weights=uw1}: NN i1 w1 o1 c1 t [s1] [ps1] [ts1])
                     ({forward=f2, backward=b2,
                        pickle=p2, specs=sp2,
                        functor=fun2, w_init=wi2,
                        update_weights=uw2}: NN o1 w2 o2 c2 t [s2] [ps2] [ts2])
                      : NN i1 (w1,w2) (o2) (c1,c2) t [s1+s2] [ps1+ps2] [ts1+ts2]=
  ...
  let size 'w 'g 'i 'o [s] [p] [ts] (_: NN i w o g t [s] [p] [ts]): i64 = p

  let init_weights 'w 'g 'i 'o [s] [p] [ts]
                   (nn: NN i w o g t [s] [p] [ts]) =
    { weights = nn.w_init () }

  let predict  [K] 'i 'w 'g 'o [s] [ps] [ts]
               (nn: NN i w o g t [s] [ps] [ts])
               (ws: weights w)
               (input: [K]i)
               ({f=class, fd = _}: activation_func o) =
    let ws = get_weights ws
    let (_, output) = nn.forward K false ws input
    in map class output


  let accuracy [K] 'w 'g 'i 'o [s] [ps] [ts]
               (nn: NN i w o g t [s] [ps] [ts])
               (ws: weights w)
               (input: [K]i)
               (labels: [K]o)
